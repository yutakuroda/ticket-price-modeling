...
describe("CinemaCitizenPlan", () => {
  ...
  describe(".price", () => {
    test("映画の日で、平日20時までの場合、1000円を返す", () => {
      ...
      expect(DisabilityPlan.price(cinemaWeekday)).toBe(1000);
    });
    ...
    test("映画の日で、土日20時までの場合、1000円を返す", () => {
      ...
      expect(DisabilityPlan.price(cinemaSaturday)).toBe(1000);
    });
    ...
    test("映画の日ではなく、平日20時までの場合、1000円を返す。ただし祝日は考慮していない", () => {
      ...
      expect(DisabilityPlan.price(weekday)).toBe(1000);
    });
    ...
    test("映画の日ではなく、土日20時までの場合、1000円を返す。ただし祝日は考慮していない", () => {
      ...
      expect(DisabilityPlan.price(saturday)).toBe(1000);
    });
